#include <cuda_bf16.h>
#include <cuda_fp16.h>
#include <cuda_fp8.h>
#include <float.h>
#include <cuda_runtime.h>
#include <device_launch_parameters.h>

#include "softmax.cuh"

#define WARP_SIZE 32
#define INT4(value) (reinterpret_cast<int4 *>(&(value))[0])
#define FLOAT4(value) (reinterpret_cast<float4 *>(&(value))[0])
#define HALF2(value) (reinterpret_cast<half2 *>(&(value))[0])
#define BFLOAT2(value) (reinterpret_cast<__nv_bfloat162 *>(&(value))[0])
#define LDST128BITS(value) (reinterpret_cast<float4 *>(&(value))[0])

// FP32
// DS required for Online Softmax
struct __align__(8) MD 
{
	float m;
	float d;
};

// Warp Reduce for Online Softmax
template <const int warp_size = WARP_SIZE>
__device__ __forceinline__ MD warp_reduce_md_op(MD value)
{
	unsigned int mask = 0xffffffff;
#pragma unroll
	for (int stride = warp_size >> 1; stride >= 1; stride >>= 1)
	{
		MD other;
		other.m = __shfl_xor_sync(mask, value.m, stride);
		other.d = __shfl_xor_sync(mask, value.d, stride);

		bool value_bigger = (value.m > other.m);
		MD bigger_m = value_bigger ? value : other;
		MD smaller_m = value_bigger ? other : value;

		value.d = bigger_m.d + smaller_m.d * __expf(smaller_m.m - bigger_m.m);
		value.m = bigger_m.m;
	}

	return value;
}

// Warp Reduce Sum
template <const int warp_size = WARP_SIZE>
__device__ __forceinline__ float warp_reduce_sum_f32(float val)
{
#pragma unroll
	for (int mask = warp_size >> 1; mask >= 1; mask >>= 1)
	{
		val += __shfl_xor_sync(0xffffffff, val, mask);
	}

	return val;
}

// Warp Reduce Max
template <const int warp_size = WARP_SIZE>
__device__ __forceinline__ float warp_reduce_max_f32(float val)
{
#pragma unroll
	for (int mask = warp_size >> 1; mask >= 1; mask >>= 1)
	{
		val = fmaxf(val, __shfl_xor_sync(0xffffffff, val, mask));
	}

	return val;
}

// grid 1D block 1D, grid(N/256), block(256)
template <const int num_threads = 256>
__device__ float block_reduce_sum_f32(float val)
{
	// always <= 32 warps per block (limited by 1024 threads per block)
	constexpr int num_warps = (num_threads + WARP_SIZE - 1) / WARP_SIZE;
	int warp = threadIdx.x / WARP_SIZE;
	int lane = threadIdx.x % WARP_SIZE;
	static __shared__ float shared[num_warps];

	float value = warp_reduce_sum_f32<WARP_SIZE>(val);
	if (lane == 0) shared[warp] = value;
	__syncthreads();
	value = (lane < num_warps) ? shared[lane] : 0.0f;
	value = warp_reduce_sum_f32<num_warps>(value);
	// WRAN: need to broadcast value to all threads within warp
	value = __shfl_sync(0xffffffff, value, 0, 32);
	
	return value;
}

template <const int num_threads = 256>
__device__ float block_reduce_max_f32(float val) 
{
	// always <= 32 warps per block (limited by 1024 threads per block)
	constexpr int num_warps = (num_threads + WARP_SIZE - 1) / WARP_SIZE;
	int warp = threadIdx.x / WARP_SIZE;
	int lane = threadIdx.x % WARP_SIZE;
	static __shared__ float shared[num_warps];

	float value = warp_reduce_max_f32<WARP_SIZE>(val);
	if (lane == 0) shared[warp] = value;
	__syncthreads();
	value = (lane < num_warps) ? shared[lane] : -FLT_MAX;
	value = warp_reduce_max_f32<num_warps>(value);
	// WRAN: need to broadcast value to all threads within warp
	value = __shfl_sync(0xffffffff, value, 0, 32);

	return value;
}

// NOTE: softmax per-token
// Softmax x: (S,h), y: (S,h)
// grid(S*h/h), block(h), assume h<=1024
// one token per thread block, only support 64<=h<=1024 and 2^n
// HEAD_SIZE / KV_LEN = num_threads
template <const int num_threads = 256>
__global__ void softmax_f32_per_token_kernel(float* x, float* y, int n) 
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;

	float exp_val = (idx < n) ? expf(x[idx]) : 0.0f;
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum
	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if (idx < n) y[idx] = exp_val / exp_sum;
}

template <const int num_threads = 256 / 4>
__global__ void softmax_f32x4_per_token_kernel(float* x, float* y, int n) 
{
	int idx = (threadIdx.x + blockIdx.x * blockDim.x) * 4;

	float4 reg_x = FLOAT4(x[idx]);
	float4 reg_exp;
	reg_exp.x = (idx + 0 < n) ? expf(reg_x.x) : 0.0f;
	reg_exp.y = (idx + 1 < n) ? expf(reg_x.y) : 0.0f;
	reg_exp.z = (idx + 2 < n) ? expf(reg_x.z) : 0.0f;
	reg_exp.w = (idx + 3 < n) ? expf(reg_x.w) : 0.0f;

	float exp_val = (reg_exp.x + reg_exp.y + reg_exp.z + reg_exp.w);
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum
	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if (idx + 3 < n) {
		float4 reg_y;
		reg_y.x = reg_exp.x / (exp_sum);
		reg_y.y = reg_exp.y / (exp_sum);
		reg_y.z = reg_exp.z / (exp_sum);
		reg_y.w = reg_exp.w / (exp_sum);
		FLOAT4(y[idx]) = reg_y;
	}
}

// safe_softmax per token
template <const int num_threads = 256>
__global__ void safe_softmax_f32_per_token_kernel(float* x, float* y, int n) 
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;

	float val = (idx < n) ? x[idx] : -FLT_MAX;
	float max_val = block_reduce_max_f32<num_threads>(val); // block max
	float exp_val = (idx < n) ? expf(x[idx] - max_val) : 0.0f;
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum
	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if (idx < n) y[idx] = exp_val / exp_sum;
}

template <const int num_threads = 256 / 4>
__global__ void safe_softmax_f32x4_per_token_kernel(float* x, float* y, int n) 
{
	int idx = (threadIdx.x + blockIdx.x * blockDim.x) * 4;

	float4 reg_x = FLOAT4(x[idx]);
	reg_x.x = (idx + 0 < n) ? reg_x.x : -FLT_MAX;
	reg_x.y = (idx + 1 < n) ? reg_x.y : -FLT_MAX;
	reg_x.z = (idx + 2 < n) ? reg_x.z : -FLT_MAX;
	reg_x.w = (idx + 3 < n) ? reg_x.w : -FLT_MAX;
	float val = reg_x.x;
	val = fmaxf(val, reg_x.y);
	val = fmaxf(val, reg_x.z);
	val = fmaxf(val, reg_x.w);
	float max_val = block_reduce_max_f32<num_threads>(val); // block max

	float4 reg_exp;
	reg_exp.x = (idx + 0 < n) ? expf(reg_x.x - max_val) : 0.0f;
	reg_exp.y = (idx + 1 < n) ? expf(reg_x.y - max_val) : 0.0f;
	reg_exp.z = (idx + 2 < n) ? expf(reg_x.z - max_val) : 0.0f;
	reg_exp.w = (idx + 3 < n) ? expf(reg_x.w - max_val) : 0.0f;

	float exp_val = (reg_exp.x + reg_exp.y + reg_exp.z + reg_exp.w);
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum
	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if (idx + 3 < n) {
		float4 reg_y;
		reg_y.x = reg_exp.x / (exp_sum);
		reg_y.y = reg_exp.y / (exp_sum);
		reg_y.z = reg_exp.z / (exp_sum);
		reg_y.w = reg_exp.w / (exp_sum);
		FLOAT4(y[idx]) = reg_y;
	}
}

template <const int num_threads = 256>
__global__ void safe_softmax_f16_f32_per_token_kernel(half* x, half* y, int n) 
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;

	float val = (idx < n) ? __half2float(x[idx]) : -FLT_MAX;
	float max_val = block_reduce_max_f32<num_threads>(val); // block max
	float exp_val = (idx < n) ? expf(val - max_val) : 0.0f;
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum
	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if (idx < n) y[idx] = __float2half_rn(exp_val / exp_sum);
}

template <const int num_threads = 256>
__global__ void safe_softmax_f16x2_f32_per_token_kernel(half* x, half* y, int n) 
{
	int idx = (threadIdx.x + blockIdx.x * blockDim.x) * 2;

	float2 reg_x = __half22float2(HALF2(x[idx]));
	float max_val = -FLT_MAX;
	max_val = ((idx + 0) < n) ? fmaxf(reg_x.x, max_val) : -FLT_MAX;
	max_val = ((idx + 1) < n) ? fmaxf(reg_x.y, max_val) : -FLT_MAX;
	max_val = block_reduce_max_f32<num_threads>(max_val); // block max

	float2 reg_exp;
	reg_exp.x = ((idx + 0) < n) ? expf(reg_x.x - max_val) : 0.0f;
	reg_exp.y = ((idx + 1) < n) ? expf(reg_x.y - max_val) : 0.0f;

	float exp_val = reg_exp.x + reg_exp.y;
	float exp_sum = block_reduce_sum_f32<num_threads>(exp_val); // block sum

	float2 reg_y;
	reg_y.x = reg_exp.x / (exp_sum);
	reg_y.y = reg_exp.y / (exp_sum);

	// e^x_i/sum(e^x_0,...,e^x_n-1)
	if ((idx + 1) < n) HALF2(y[idx]) = __float22half2_rn(reg_y);
}

template <const int num_threads = 256>
__global__ void safe_softmax_f16x8_pack_f32_per_token_kernel(half* x, half* y, int n) 
{
	int idx = (threadIdx.x + blockIdx.x * blockDim.x) * 8;
	// temporary register(memory), .local space in ptx, addressable
	half pack_x[8], pack_y[8]; // 8x16 bits=128 bits
	// reinterpret as float4 and load 128 bits in 1 memory issue
	LDST128BITS(pack_x[0]) = LDST128BITS(x[idx]); // load 128 bits

	float max_val = -FLT_MAX;
#pragma unroll
	for (int i = 0; i < 8; ++i) 
	{
		max_val = fmaxf(__half2float(pack_x[i]), max_val);
	}
	max_val = block_reduce_max_f32<num_threads>(max_val); // block max

	float exp_sum = 0.0f;
#pragma unroll
	for (int i = 0; i < 8; ++i) 
	{
		float exp_val = expf(__half2float(pack_x[i]) - max_val);
		exp_sum += (((idx + i) < n) ? exp_val : 0.0f);
	}
	exp_sum = block_reduce_sum_f32<num_threads>(exp_sum); // block sum

#pragma unroll
	for (int i = 0; i < 8; ++i) 
	{
		// e^x_i/sum(e^x_0,...,e^x_n-1)
		float exp_val = expf(__half2float(pack_x[i]) - max_val);
		pack_y[i] = __float2half_rn(exp_val / exp_sum);
	}
	// reinterpret as float4 and store 128 bits in 1 memory issue.
	if ((idx + 7) < n) {
		LDST128BITS(y[idx]) = LDST128BITS(pack_y[0]);
	}
	// TODO: support non 8-multiple K here
}

template <const int num_threads = 256>
__global__ void online_safe_softmax_f32_per_token_kernel(float* x, float* y, int n)
{
	// reference: https://arxiv.org/pdf/1805.02867 (Online normalizer calculation
	// for softmax)
	int local_tid = threadIdx.x;
	int global_tid = threadIdx.x + blockIdx.x * num_threads;
	const int warp_num = num_threads / WARP_SIZE;
	int warp_id = local_tid / WARP_SIZE;
	int lane_id = local_tid % WARP_SIZE;
	MD val;
	val.m = global_tid < n ? x[global_tid] : -FLT_MAX;
	val.d = global_tid < n ? 1.0f : 0.0f;

	__shared__ MD shared[warp_num];
	MD res = warp_reduce_md_op<WARP_SIZE>(val);

	if (lane_id == 0) shared[warp_id] = res;
	__syncthreads();

	if (local_tid < WARP_SIZE) {
		MD block_res = shared[local_tid];
		block_res = warp_reduce_md_op<warp_num>(block_res);
		if (local_tid == 0) {
			shared[0] = block_res;
		}
	}
	__syncthreads();

	MD final_res = shared[0];
	float d_total_inverse = __fdividef(1.0f, final_res.d);
	if (global_tid < n) {
		y[global_tid] = __expf(x[global_tid] - final_res.m) * d_total_inverse;
	}
}

template <const int num_threads = 256 / 4>
__global__ void online_safe_softmax_f32x4_pack_per_token_kernel(float* x, float* y, int n) 
{
	// reference: https://arxiv.org/pdf/1805.02867 (Online normalizer calculation
	// for softmax)
	int local_tid = threadIdx.x;
	int global_tid = (local_tid + blockIdx.x * num_threads) * 4;

	const int warp_num = num_threads / WARP_SIZE;
	int warp_id = local_tid / WARP_SIZE;
	int lane_id = local_tid % WARP_SIZE;
	// compare local max value
	float4 val = FLOAT4((x)[global_tid]);
	float local_m = fmaxf(fmaxf(val.x, val.y), fmaxf(val.z, val.w));
	float local_d = __expf(val.x - local_m) + __expf(val.y - local_m) +
		__expf(val.z - local_m) + __expf(val.w - local_m);

	MD local_md = { local_m, local_d };
	MD res = warp_reduce_md_op<WARP_SIZE>(local_md);
	__shared__ MD shared[warp_num];

	if (lane_id == 0) shared[warp_id] = res;
	__syncthreads();
	// do block reduce
	if (local_tid < WARP_SIZE) {
		MD block_res = shared[local_tid];
		block_res = warp_reduce_md_op<warp_num>(block_res);
		if (local_tid == 0) shared[0] = block_res;
	}
	__syncthreads();
	// write back
	MD final_res = shared[0];
	float d_total_inverse = __fdividef(1.0f, final_res.d);
	if (global_tid < n) {
		float4 reg_y;
		reg_y.x = __expf(val.x - final_res.m) * d_total_inverse;
		reg_y.y = __expf(val.y - final_res.m) * d_total_inverse;
		reg_y.z = __expf(val.z - final_res.m) * d_total_inverse;
		reg_y.w = __expf(val.w - final_res.m) * d_total_inverse;
		FLOAT4((y)[global_tid]) = reg_y;
	}
}

static inline void check_args_2d(const void* x, const void* y, int S, int H, int* N) 
{
	if (!x || !y || !N || S <= 0 || H <= 0) return ;
	long long n64 = 1LL * S * H;
	if (n64 > INT32_MAX) return ;
	*N = static_cast<int>(n64);
}

#define LAUNCH_H_CASE(KERNEL, HVAL) \
  case HVAL: KERNEL<HVAL><<<grid, dim3(HVAL), 0, stream>>>(x, y, N); break;

#define LAUNCH_H_DIV_CASE(KERNEL, HVAL, DIV) \
  case HVAL: KERNEL<HVAL / DIV><<<grid, dim3(HVAL / DIV), 0, stream>>>(x, y, N); break;

void softmax_f32_per_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 32)
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 64)
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 128)
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 256)
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 512)
		LAUNCH_H_CASE(softmax_f32_per_token_kernel, 1024)
		default: return ;
	}
}

void softmax_f32x4_per_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 32, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 64, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 128, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 256, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 512, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 1024, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 2048, 4)
		LAUNCH_H_DIV_CASE(softmax_f32x4_per_token_kernel, 4096, 4)
		default: return ;
	}
}

void safe_softmax_f32_per_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 32)
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 64)
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 128)
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 256)
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 512)
		LAUNCH_H_CASE(safe_softmax_f32_per_token_kernel, 1024)
		default: return ;
	}
}

void safe_softmax_f32x4_pre_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 32, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 64, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 128, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 256, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 512, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 1024, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 2048, 4)
		LAUNCH_H_DIV_CASE(safe_softmax_f32x4_per_token_kernel, 4096, 4)
		default: return ;
	}
}

void safe_softmax_f16_f32_per_token(half * x, half * y, int S, int H, cudaStream_t stream)
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 32)
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 64)
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 128)
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 256)
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 512)
		LAUNCH_H_CASE(safe_softmax_f16_f32_per_token_kernel, 1024)
		default: return ;
	}
}

void safe_softmax_f16x2_f32_per_token(half * x, half * y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N); 
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 32, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 64, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 128, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 256, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 512, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 1024, 2)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x2_f32_per_token_kernel, 2048, 2)
		default: return ;
	}
}

void safe_softmax_f16x8_pack_f32_per_token(half * x, half * y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 32, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 64, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 128, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 256, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 512, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 1024, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 2048, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 4096, 8)
		LAUNCH_H_DIV_CASE(safe_softmax_f16x8_pack_f32_per_token_kernel, 8192, 8)
		default: return ;
	}
}

void online_safe_softmax_f32_per_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 32)
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 64)
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 128)
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 256)
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 512)
		LAUNCH_H_CASE(online_safe_softmax_f32_per_token_kernel, 1024)
		default: return ;
	}
}

void online_safe_softmax_f32x4_pack_per_token(float* x, float* y, int S, int H, cudaStream_t stream) 
{
	int N; 
	check_args_2d(x, y, S, H, &N);
	dim3 grid(S);
	switch (H) 
	{
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 128, 4)
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 256, 4)
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 512, 4)
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 1024, 4)
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 2048, 4)
		LAUNCH_H_DIV_CASE(online_safe_softmax_f32x4_pack_per_token_kernel, 4096, 4)
		default: return ;
	}
}