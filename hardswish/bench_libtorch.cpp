	...
	for (int S : Ss)
	{
		for (int K : Ks)
		{
			...
			auto ms_f32 = benchmark_ms([&]() {
				hardswish_f32(x.data_ptr<float>(), y.data_ptr<float>(), ndim, S, K, N, stream);
			}, stream);
			print_result("out_f32", y, ms_f32);

			auto ms_f32x4 = benchmark_ms([&]() {
				hardswish_f32x4(x.data_ptr<float>(), y.data_ptr<float>(), ndim, S, K, N, stream);
			}, stream);
			...
			auto ms_f16 = benchmark_ms([&]() {
				hardswish_f16(x16.data_ptr(), y16.data_ptr(), ndim, S, K, N, stream);
			}, stream);
			print_result("out_f16", y16, ms_f16);

			auto ms_f16x2 = benchmark_ms([&]() {
				hardswish_f16x2(x16.data_ptr(), y16.data_ptr(), ndim, S, K, N, stream);
			}, stream);
			print_result("out_f16x2", y16, ms_f16x2);

			auto ms_f16x8 = benchmark_ms([&]() {
				hardswish_f16x8(x16.data_ptr(), y16.data_ptr(), ndim, S, K, N, stream);
			}, stream);
			print_result("out_f16x8", y16, ms_f16x8);

			auto ms_f16x8p = benchmark_ms([&]() {
				hardswish_f16x8_pack(x16.data_ptr(), y16.data_ptr(), ndim, S, K, N, stream);
			}, stream);
