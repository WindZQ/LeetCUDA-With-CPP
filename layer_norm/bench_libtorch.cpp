...
Tensor naive_layer_norm(const Tensor& x, float g, float b) 
{
	auto s_mean = x.mean(1, true);
	auto s_std = x.std(1, true, true);
	auto s_variance = 1.0 / s_std;
	return ((x - s_mean) * s_variance) * g + b;
}
...
void benchmark_case(int N, int K) 
{
	...
	auto bench_naive = [&](const Tensor& in, float g, float b) -> Tensor {
		return naive_layer_norm(in, g, b);
	};
	...
	if (K <= 4096) {
		...
		auto bench_layer_norm_f32 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f32(in.data_ptr<float>(), o.data_ptr<float>(), g, b, N, K, stream);
		};
		auto bench_layer_norm_f32x4 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f32x4(in.data_ptr<float>(), o.data_ptr<float>(), g, b, N, K, stream);
		};

		if (K == 512 || K == 1024) {
			run_benchmark_out(bench_layer_norm_f32, x, "f32", out);
			run_benchmark_out(bench_layer_norm_f32x4, x, "f32x4", out);
		...
		auto bench_layer_norm_f16_f16 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f16_f16(
				reinterpret_cast<half*>(in.data_ptr<at::Half>()),
				reinterpret_cast<half*>(o.data_ptr<at::Half>()),
				g, b, N, K, stream);
		};
		auto bench_layer_norm_f16_f32 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f16_f32(
				reinterpret_cast<half*>(in.data_ptr<at::Half>()),
				reinterpret_cast<half*>(o.data_ptr<at::Half>()),
				g, b, N, K, stream);
		};
		auto bench_layer_norm_f16x2_f16 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f16x2_f16(
				reinterpret_cast<half*>(in.data_ptr<at::Half>()),
				reinterpret_cast<half*>(o.data_ptr<at::Half>()),
				g, b, N, K, stream);
		};
		auto bench_layer_norm_f16x8_f16 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f16x8_f16(
				reinterpret_cast<half*>(in.data_ptr<at::Half>()),
				reinterpret_cast<half*>(o.data_ptr<at::Half>()),
				g, b, N, K, stream);
		};
		auto bench_layer_norm_f16x8_pack_f16 = [&](const Tensor& in, Tensor& o, float g, float b) {
			layer_norm_f16x8_pack_f16(
				reinterpret_cast<half*>(in.data_ptr<at::Half>()),
				reinterpret_cast<half*>(o.data_ptr<at::Half>()),
				g, b, N, K, stream);
		};
		auto bench_layer_norm_f16x8_pack_f32 = [&](const Tensor& in, Tensor& o, float g, float b) {
		...
		if (K == 512 || K == 1024) {
			run_benchmark_out(bench_layer_norm_f16_f16, x_f16, "f16f16", out_f16);
			run_benchmark_out(bench_layer_norm_f16_f32, x_f16, "f16f32", out_f16);
			run_benchmark_out(bench_layer_norm_f16x2_f16, x_f16, "f16x2f16", out_f16);
			run_benchmark_out(bench_layer_norm_f16x8_f16, x_f16, "f16x8f16", out_f16);
			run_benchmark_out(bench_layer_norm_f16x8_pack_f16, x_f16, "f16x8packf16", out_f16);
			run_benchmark_out(bench_layer_norm_f16x8_pack_f32, x_f16, "f16x8packf32", out_f16);
