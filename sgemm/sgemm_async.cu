...
template <const int BM = 64,
    ...
    const int TN = 4,
    const int OFFSET = 0>
__global__ void sgemm_t_8x4_sliced_k16_f32x4_bcf_dbuf_kernel(float* a, float* b, float* c, const int M, const int N, const int K)
{
    ...
    int load_a_gmem_m = load_a_smem_m + by * BM;
    int load_b_gmem_n = load_b_smem_n + bx * BN;

    {
        int load_a_gmem_addr = load_a_smem_k + load_a_gmem_m * K;
        int load_b_gmem_addr = load_b_gmem_n + load_b_smem_k * N;
...
#pragma unroll
        for (int i = 0; i < 8; ++i)
        {
            s_a[0][i + load_a_smem_k][load_a_smem_m] = r_load_a[i];
        }
    }
    __syncthreads();
    ...
    for (int bk = 1; bk < (K + BK - 1) / BK; ++bk)
    {
        ...
        int load_a_gmem_addr = load_a_gmem_k + load_a_gmem_m * K;
        int load_b_gmem_k = load_b_smem_k + bk * BK;
        int load_b_gmem_addr = load_b_gmem_n + load_b_gmem_k * N;
#pragma unroll 
        for (int i = 0; i < 8; i += 4)
        {
            FLOAT4(s_b[smem_sel_next][load_b_smem_k][i + load_b_smem_n]) = FLOAT4(b[i + load_b_gmem_addr]);
            FLOAT4(r_load_a[i]) = FLOAT4(a[i + load_a_gmem_addr]);
        }
#pragma unroll
...
#pragma unroll
        for (int i = 0; i < 8; ++i) 
        {
            s_a[smem_sel_next][i + load_a_smem_k][load_a_smem_m] = r_load_a[i];
        }
...
#pragma unroll
    for (int i = 0; i < TM; i++) 
    {
        int store_c_gmem_m = i + by * BM + ty * TM;
        int store_c_gmem_n = bx * BN + tx * TN;
        int store_c_gmem_addr = store_c_gmem_n + store_c_gmem_m * N;
...
template <const int BM = 64,
    ...
    const int TN = 4,
    const int OFFSET = 0>
 __global__ void sgemm_t_8x4_sliced_k16_f32x4_bcf_dbuf_async_kernel(float* a, float* b, float* c, const int M, const int N, const int K) 
{
    // block(BN/TN, BM/TM) -> (x=16,y=8), 128 threads
    const int bx = blockIdx.x;
    const int by = blockIdx.y;
...
template <const int BM = 128,
    ...
    const int TN = 8,
    const int OFFSET = 0>
__global__ void sgemm_t_8x8_sliced_k16_f32x4_bcf_dbuf_kernel(float* a, float* b, float* c, const int M, const int N, const int K) 
{
...
template <const int BM = 128,
    ...
    const int TN = 8,
    const int OFFSET = 0>
__global__ void sgemm_t_8x8_sliced_k16_f32x4_bcf_dbuf_async_kernel(float* a, float* b, float* c, const int M, const int N, const int K)
{
    // block(BN/TN, BM/TM) -> (x=16,y=16), 256 threads
    const int bx = blockIdx.x;
    const int by = blockIdx.y;
...
#pragma unroll
        for (int i = 0; i < 8; i += 4) 
        {
            FLOAT4(s_b[smem_sel_next][load_b_smem_k][load_b_smem_n + i]) = FLOAT4(b[load_b_gmem_addr + i]);
            FLOAT4(r_load_a[i]) = FLOAT4(a[load_a_gmem_addr + i]);
        }
...
#pragma unroll
    for (int i = 0; i < TM / 2; ++i) 
    {
        int store_c_gmem_m = i + by * BM + ty * TM / 2;
        int store_c_gmem_n = bx * BN + tx * TN / 2;
        int store_c_gmem_addr = store_c_gmem_n + store_c_gmem_m * N;
        FLOAT4(c[store_c_gmem_addr]) = FLOAT4(r_c[i][0]);
        FLOAT4(c[store_c_gmem_addr + BN / 2]) = FLOAT4(r_c[i][4]);
...
#pragma unroll
    for (int i = 0; i < TM / 2; ++i) 
    {
        int store_c_gmem_m = i + by * BM + BM / 2 + ty * TM / 2;
        int store_c_gmem_n = bx * BN + tx * TN / 2;
        int store_c_gmem_addr = store_c_gmem_n + store_c_gmem_m * N;
        FLOAT4(c[store_c_gmem_addr]) = FLOAT4(r_c[i + TM / 2][0]);
        FLOAT4(c[store_c_gmem_addr + BN / 2]) = FLOAT4(r_c[i + TM / 2][4]);
...
template <const int BM = 128,
    ...
    const int TN = 16,
    const int OFFSET = 0>
__global__ void sgemm_t_8x16_sliced_k16_f32x4_bcf_dbuf_kernel(float* a, float* b, float* c, const int M, const int N, const int K) 
{
    // block(BN/TN, BM/TM) -> (x=16,y=16), 256 threads
    const int bx = blockIdx.x;
    const int by = blockIdx.y;
...
#pragma unroll
        ...
        {
            FLOAT4(s_b[0][load_b_smem_k][i + load_b_smem_n]) = (FLOAT4(b[i + load_b_gmem_addr]));
        }
        // a: load 8 values per thread
#pragma unroll
        for (int i = 0; i < 8; i += 4) 
...
#pragma unroll
        for (int i = 0; i < 16; i += 4) 
        {
            FLOAT4(s_b[smem_sel_next][load_b_smem_k][i + load_b_smem_n]) =
                (FLOAT4(b[i + load_b_gmem_addr]));
        }
...
#pragma unroll
            for (int r = 0; r < 8; r += 4)
            {
                FLOAT4(r_comp_a[r]) = FLOAT4(s_a[smem_sel][tk][r + ty * TM]);
            }
#pragma unroll
            for (int r = 0; r < 16; r += 4) 
            {
                FLOAT4(r_comp_b[r]) = FLOAT4(s_b[smem_sel][tk][r + tx * TN]);
            }
...
#pragma unroll
        ...
        {
            int store_c_gmem_m = m + by * BM + ty * TM;
            int store_c_gmem_n = n + bx * BN + tx * TN;
            int store_c_gmem_addr = store_c_gmem_n + store_c_gmem_m * N;
...
template <const int BM = 128,
    ...
    const int TN = 16,
    const int OFFSET = 0>
__global__ void sgemm_t_8x16_sliced_k16_f32x4_bcf_dbuf_async_kernel(float* a, float* b, float* c, const int M, const int N, const int K) 
{
    // block(BN/TN, BM/TM) -> (x=16,y=16), 256 threads
    const int bx = blockIdx.x;
    const int by = blockIdx.y;
