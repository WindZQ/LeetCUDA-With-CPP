 ...
 __global__ void sgemm_wmma_m16n16k8_mma4x2_warp2x4_stages_kernel(float* A, float* B, float* C, int M, int N, int K) 
{
    ...
    constexpr int BM_local = WMMA_M * WMMA_TILE_M * WARP_TILE_M;
    constexpr int BN_local = WMMA_N * WMMA_TILE_N * WARP_TILE_N;
    constexpr int BK = WMMA_K;
    __shared__ float s_a[K_STAGE][BM_local][BK + A_PAD], s_b[K_STAGE][BK][BN_local + B_PAD];
...
#pragma unroll
    for (int k = 0; k < (K_STAGE - 1); ++k)  // 0, 1
    {
        int load_gmem_a_k = load_smem_a_k + k * WMMA_K; // global col of a
        int load_gmem_a_addr = load_gmem_a_k + load_gmem_a_m * K;
        int load_gmem_b_k = load_smem_b_k + k * WMMA_K; // global row of b
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i)
        {
            // load 2 tiles -> reg, smem a -> frags a, warp_m 0~3
            const int warp_smem_a_m = warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
            float* sa_ptr = s_a + smem_sel * s_a_stage_offset +
                warp_smem_a_m * (BK + A_PAD);
            wmma::load_matrix_sync(A_frag[i], sa_ptr, BK + A_PAD);
        }
...
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j)
        {
            // load 4 tiles -> reg, smem b -> frags b, warp_n 0~2
            const int warp_smem_b_n = warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
            float* sb_ptr = s_b + smem_sel * s_b_stage_offset +
                warp_smem_b_n;
            wmma::load_matrix_sync(B_frag[j], sb_ptr, BN + B_PAD);
        }
...
#pragma unroll
            for (int i = 0; i < WARP_TILE_M; ++i)
            {
                // load 2 tiles -> reg, smem a -> frags a, warp_m 0~3
                const int warp_smem_a_m = warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
                float* sa_ptr = s_a + stage_sel * s_a_stage_offset +
                    warp_smem_a_m * (BK + A_PAD);
                wmma::load_matrix_sync(A_frag[i], sa_ptr, BK + A_PAD);
            }
...
#pragma unroll
            for (int j = 0; j < WARP_TILE_N; ++j)
            {
                // load 4 tiles -> reg, smem b -> frags b, warp_n 0~2
                const int warp_smem_b_n = warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
                float* sb_ptr = s_b + stage_sel * s_b_stage_offset +
                    warp_smem_b_n;
                wmma::load_matrix_sync(B_frag[j], sb_ptr, BN + B_PAD);
            }
