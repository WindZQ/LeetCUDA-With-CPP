...
__global__ void sgemm_sliced_k_f32_kernel(float* a, float* b, float* c, int M, int N, int K)
{
	...
	int tx = threadIdx.x;
	int ty = threadIdx.y;
	int tid = tx + threadIdx.y * blockDim.x; // tid within the block
...
#pragma unroll
		for (int k = 0; k < BK; ++k)
		{
			sum += s_a[load_smem_a_m][k] * s_b[load_smem_b_n][k];
		}
		__syncthreads();
	}

	int store_geme_c_addr = load_gmem_a_m * N + load_gmem_b_n;
	c[store_geme_c_addr] = sum;
}
...
__global__ void sgemm_t_8x8_sliced_k_f32x4_kernel(float* a, float* b, float* c, int M, int N, int K)
{
	...
	int load_gmem_a_m = load_smem_a_m + by * BM; // global row of a and c
	int load_gmem_b_n = load_smem_b_n + bx * BN; // global col of b and c
	float r_c[TM][TN] = { 0.0 }; // 8x8
	// 2. 先对K进行分块，每块BK大小
	for (int bk = 0; bk < (K + BK - 1) / BK; ++bk)
	{
		// 加载数据到共享内存smem s_a BM*BK 128*8 vectorize float4
		int load_gmem_a_k = load_smem_a_k + bk * BK; // global col of a
		int load_gmem_a_addr = load_gmem_a_k + load_gmem_a_m * K;
		FLOAT4(s_a[load_smem_a_m][load_smem_a_k]) = FLOAT4(a[load_gmem_a_addr]);
		// 加载数据到共享内存smem s_b BK*BN 8*128 vectorize float4
		int load_gmem_b_k = load_smem_b_k + bk * BK; // global row of b
		int load_gmem_b_addr = load_gmem_b_n + load_gmem_b_k * N;
...
#pragma unroll 
				for (int n = 0; n < TN; ++n)
				{
					...
					int comp_smem_a_m = m + ty * TM; // 128*8 128/TM(8)=16 M方向 16线程
					int comp_smem_b_n = n + tx * TN; // 8*128 128/TN(8)=16 N方向 16线程
...
#pragma unroll 
	for (int m = 0; m < TM; ++m)
	{
		int store_gmem_c_m = m + by * BM + ty * TM;
#pragma unroll 
		for (int n = 0; n < TN; n += 4)
		{
			int store_gmem_c_n = n + bx * BN + tx * TN;
...
template <const int BM = 128, const int BN = 128, const int BK = 8,
	const int TM = 8, const int TN = 8, const int OFFSET = 0>
__global__ void sgemm_t_8x8_sliced_k_f32x4_bcf_dbuf_kernel(float* a, float* b, float* c,
		const int M, const int N, const int K)
{
	...
	// 1）主循环从bk = 1开始，第一次数据加载在主循环之前，最后一次计算在主循环之后，这是pipeline的特点决定
	// 2）由于计算和下一次访存使用的Shared Memory不同，因此主循环中每次循环只需要一次__syncthreads()即可
	// 3）由于GPU不能向CPU那样支持乱序执行，主循环中需要先将下一次循环计算需要的Gloabal Memory中的数据load 到寄存器，
	// 然后进行本次计算，之后再将load到寄存器中的数据写到Shared Memory，这样在LDG指令向Global Memory做load时，
	// 不会影响后续FFMA及其它运算指令的launch执行，也就达到了Double Buffering的目的
	{
...
#pragma unroll
		...
		// 对比非double buffers版本，此处不需要__syncthreads()，总共节省了((K + BK - 1) / BK) - 1 次block内的同步操作。比如，bk=1时，HFMA计算
		// 使用的是s_a[0]和s_b[0]，因此，和s_a[1]和s_b[1]的加载是没有依赖关系的。
		// 从global内存到s_a[1]和s_b[1]和HFMA计算可以并行。s_a[1]和s_b[1]用于加载下一块BK需要的数据到共享内存。
...
#pragma unroll
	for (int i = 0; i < TM / 2; i++) 
	{
		int store_c_gmem_m = i + ty * TM / 2 + by * BM ;
		int store_c_gmem_n = tx * TN / 2 + bx * BN ;
...
#pragma unroll
	for (int i = 0; i < TM / 2; i++) 
	{
		int store_c_gmem_m = i + ty * TM / 2 + by * BM + BM / 2;
		int store_c_gmem_n = tx * TN / 2 + bx * BN;
		int store_c_gmem_addr = store_c_gmem_n + store_c_gmem_m * N;
		FLOAT4(c[store_c_gmem_addr]) = FLOAT4(r_c[i + TM / 2][0]);
		FLOAT4(c[store_c_gmem_addr + BN / 2]) = FLOAT4(r_c[i + TM / 2][4]);
