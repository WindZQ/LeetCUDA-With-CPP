...
__device__ __forceinline__ half swish_half(half x)
{
	return __hmul(x, __hdiv(__float2half(1.0f),
		__hadd(__float2half(1.0f), hexp_compat(__hneg(x)))));
}
	...
	cudaStream_t stream)
{
	...
	swish_f32_kernel << <grid, block, 0, stream >> > (x, y, (int)N);
}
	...
	cudaStream_t stream)
{
	if ((N % 4) != 0 || (ndim == 2 && (K % 4) != 0)) {
		swish_f32(x, y, ndim, S, K, N, stream);
	}
	dim3 grid, block;
	launch_1d_or_2d(ndim, S, K, 4, grid, block, N);
	swish_f32x4_kernel << <grid, block, 0, stream >> > (x, y, (int)N);
}
	...
	cudaStream_t stream)
{
	...
	dim3 grid, block;
	launch_1d_or_2d(ndim, S, K, 1, grid, block, N);
	swish_f16_kernel << <grid, block, 0, stream >> > (xp, yp, (int)N);
}
	...
	cudaStream_t stream)
{
	if ((N % 2) != 0 || (ndim == 2 && (K % 2) != 0)) {
		swish_f16(x, y, ndim, S, K, N, stream);
	}
	half* xp = reinterpret_cast<half*>(x);
	half* yp = reinterpret_cast<half*>(y);
	...
	swish_f16x2_kernel << <grid, block, 0, stream >> > (xp, yp, (int)N);
}
	...
	cudaStream_t stream)
{
	...
	swish_f16x8_kernel << <grid, block, 0, stream >> > (xp, yp, (int)N);
}
	...
	cudaStream_t stream)
{
	...
	dim3 grid, block;
	launch_1d_or_2d(ndim, S, K, 8, grid, block, N);
	swish_f16x8_pack_kernel << <grid, block, 0, stream >> > (xp, yp, (int)N);
}